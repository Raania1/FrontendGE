...
interface Pack {
  id: string;
  title: string;
  prestataireid: string; // Lowercase to match Prisma schema
...
@Component({
  selector: 'app-reservation-pkor',
  standalone: true,
  imports: [NavbarORComponent, FormsModule, CommonModule, RouterLink, FontAwesomeModule],
  templateUrl: './reservation-pkor.component.html',
  styleUrls: ['./reservation-pkor.component.css'],
})
export class ReservationPKORComponent implements OnInit {
  ...
  fetchOrganizerData(): void {
    ...
    const id = user.Id as string | undefined; // Adjust type to string to match backend
    console.log('User ID from localStorage:', id);

    if (id) {
      this.organizerService.getPackReservationsById(id).subscribe({
        next: (response: any) => {
          console.log('API response:', response);

          // Extract pack reservations from the response
          this.mesReservations = response.packReservations || [];
          console.log('Pack Reservations:', this.mesReservations);

          // Log each reservation for debugging
          this.mesReservations.forEach(res => {
            console.log('Reservation ID:', res.id, 'Pack:', res.Pack);
          });

          // No need to filter for Pack since the backend already ensures packid is not null
        },
        error: (error: any) => {
      ...
      this.errorMessage = 'Utilisateur non trouvé. Veuillez vous connecter.';
      setTimeout(() => this.errorMessage = '', 4000);
    }
  }
  ...
  get filteredReservations(): Reservation[] {
    return this.mesReservations
      .filter(res => res.Pack) // Redundant but kept for safety
      .filter(res => {
        const matchesStatus =
          this.selectedStatus === 'all' || res.Status.toLowerCase() === this.selectedStatus;
        const matchesSearch =
          !this.searchQuery || res.Pack.title.toLowerCase().includes(this.searchQuery.toLowerCase());
